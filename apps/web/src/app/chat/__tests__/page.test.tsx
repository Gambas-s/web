...
const MOCK_SSE = `data: ${JSON.stringify({ chunk: "ㅁㅊㅋㅋㅋ\n잼컨 발생 빨리 말해줘" })}\ndata: [DONE]\n`;
...
beforeAll(() => {
  ...
  global.fetch = vi.fn((url: string | URL | Request, options?: RequestInit) => {
    ...
    if (urlStr.includes("/api/session?sessionId=")) {
      return Promise.resolve(new Response(JSON.stringify({ messages: [] }), {
        status: 200, headers: { "Content-Type": "application/json" },
      }));
    }
    if (urlStr.includes("/api/session") && options?.method === "POST") {
      return Promise.resolve(new Response(JSON.stringify({ sessionId: "test-session" }), {
        status: 200, headers: { "Content-Type": "application/json" },
      }));
    }
    if (urlStr.includes("/api/chat")) {
      const stream = new ReadableStream({
        start(controller) {
          controller.enqueue(new TextEncoder().encode(MOCK_SSE));
          controller.close();
        },
      });
...
beforeEach(() => {
  vi.useFakeTimers();
  mockPush.mockClear();
  localStorage.setItem("gambass_session_id", "test-session");
});
...
test("600ms 후 AI 답장이 스트리밍 시작된다", async () => {
  render(<ChatPage />);
  typeAndSend("미치겠다");
...
test("스트리밍 완료 후 AI 전체 메시지가 표시된다", async () => {
  render(<ChatPage />);
  typeAndSend("퇴근하고싶어");
